...
//!
//! Provides integration with the unified Message Bus for inter-agent communication.
//! 
...
impl AgentsMessageBus {
    ...
    #[allow(dead_code)]
    pub async fn subscribe(&self, topic: &str) -> crate::error::Result<Box<dyn MessageBusSubscriber>> {
...
pub trait MessageBus: Send + Sync {
    /// Publish a message to a topic
    fn publish(&self, topic: &str, message: &[u8]) -> Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error>>> + Send + '_>>;
    /// Subscribe to a topic
    fn subscribe(&self, topic: &str) -> Pin<Box<dyn std::future::Future<Output = Result<Box<dyn MessageBusSubscriber>, Box<dyn std::error::Error>>> + Send + '_>>;
}
...
impl MessageBus for DefaultMessageBus<MemoryTransport> {
    fn publish(&self, topic: &str, message: &[u8]) -> Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error>>> + Send + '_>> {
        ...
        Box::pin(async move {
            use beebotos_message_bus::{MessageBus as ExternalMessageBus, Message};
            
            let msg = Message::new(&topic, message);
            ExternalMessageBus::publish(bus, &topic, msg).await
    ...
    fn subscribe(&self, topic: &str) -> Pin<Box<dyn std::future::Future<Output = Result<Box<dyn MessageBusSubscriber>, Box<dyn std::error::Error>>> + Send + '_>> {
        ...
        Box::pin(async move {
            ...
            let (_sub_id, mut stream) = ExternalMessageBus::subscribe(bus, &topic).await
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
...
/// 
/// SECURITY FIX: Replaced `static mut` with `OnceLock` to eliminate data race risks
/// and unsafe code blocks.
/// 
/// ARCHITECTURE FIX: Use concrete type instead of dyn trait to avoid dyn compatibility issues
...
pub fn init_message_bus<B: MessageBus + 'static>(_bus: Arc<B>) -> Result<(), &'static str> {
    ...
    GLOBAL_MESSAGE_BUS.set(default_bus)
...
/// 
/// SECURITY FIX: Provides safe access to the global message bus without unsafe blocks.
#[allow(dead_code)]
pub fn get_or_init_message_bus() -> Arc<MessageBusType> {
    GLOBAL_MESSAGE_BUS.get_or_init(|| {
        let transport = MemoryTransport::new();
        let bus: Arc<DefaultMessageBus<MemoryTransport>> = Arc::new(DefaultMessageBus::new(
            transport,
            Box::new(JsonCodec),
            None,
        ));
...
mod tests {
    ...
    async fn test_agents_message_bus() {
        ...
        // Since AgentsMessageBus::new creates its own internal bus, we just test it works
