...
use beebotos_agents::{
    Agent, AgentConfig, ExecutionResult, Plan, PlanExecutor, PlanId, PlanStatus, 
    PlanStrategy, PlanningEngine, Task, TaskComplexity, TaskType,
    testing::{
        MockPlanningEngine, MockPlanExecutor, MockRePlanner,
        helpers::*,
    },
};
...
async fn test_e2e_simple_planning_workflow() {
    ...
    // Execute the plan
    let result = agent.execute_plan(&plan).await.expect("Failed to execute plan");
...
async fn test_e2e_complex_task_auto_planning() {
    ...
    let complex_task = Task {
        ...
        input: "Design and implement a user authentication system with OAuth2, JWT, rate limiting, and audit logs. ".repeat(5),
        parameters: HashMap::new(),
    };
    
    // Verify complexity detection
    let complexity = agent.analyze_task_complexity(&complex_task).await;
    assert_eq!(complexity, TaskComplexity::Complex, "Task input length: {}", complex_task.input.len());
...
async fn test_e2e_explicit_planning_task() {
    ...
    let (output, _) = agent.handle_plan_creation_task(&plan_task).await
...
async fn test_e2e_plan_execution_task() {
    ...
    // First create a plan
    let plan = agent.create_plan("Test execution", PlanStrategy::Hybrid).await
        .expect("Failed to create plan");
    ...
    let (output, _) = agent.handle_plan_execution_task(&exec_task).await
...
async fn test_e2e_concurrent_plan_creation() {
    ...
    for i in 0..5 {
        ...
        handles.push(tokio::spawn(async move {
            agent.create_plan(&format!("Goal {}", i), PlanStrategy::Hybrid).await
        }));
    }
...
async fn test_e2e_plan_cancel_during_execution() {
    ...
    let plan = agent.create_plan("Long running task", PlanStrategy::ReAct).await
        .expect("Failed to create plan");
...
async fn test_e2e_complexity_detection_variations() {
    ...
    assert_eq!(agent.analyze_task_complexity(&task1).await, TaskComplexity::Simple);
    ...
    assert_eq!(agent.analyze_task_complexity(&task2).await, TaskComplexity::Complex);
    ...
    assert_eq!(agent.analyze_task_complexity(&task3).await, TaskComplexity::Complex);
}
...
async fn test_e2e_plan_strategy_selection() {
    ...
    for (strategy_str, expected) in strategies {
        ...
        assert!(plan.is_ok(), "Failed to create plan with strategy: {}", strategy_str);
    }
}
...
async fn test_e2e_parallel_execution_performance() {
    ...
    plan.metadata.insert("enable_parallel".to_string(), serde_json::json!(true));
    plan.metadata.insert("max_concurrency".to_string(), serde_json::json!(5));
...
async fn test_e2e_rapid_plan_create_and_cancel() {
    ...
    for i in 0..20 {
        let plan = agent.create_plan(&format!("Goal {}", i), PlanStrategy::Hybrid).await
            .expect("Failed to create plan");
