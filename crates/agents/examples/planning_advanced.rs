...
use beebotos_agents::{
    CompositeDecomposer, CompositeRePlanner, ConditionRePlanner, Decomposer,
    DecompositionContext, DecompositionStrategy, DomainDecomposer, ExecutionConfig,
    ExecutionEvent, ExecutionResult, ExecutionStrategy, FeedbackRePlanner, HybridPlanner,
    ParallelDecomposer, ParallelExecutor, Plan, PlanContext, PlanExecutor, PlanId,
    PlanStatus, PlanStep, PlanStrategy, Planner, PlanningEngine, PlanningResult,
    Priority, RePlanTrigger, RePlanner, ResourceRePlanner, SequentialExecutor,
    StepStatus, StepType, TaskDecomposer,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plan = domain_decomposer.decompose(
        "Perform security audit of the API",
        &context
    )?;
    ...
    let plan = hybrid_planner.plan(
        "Implement a new feature with fallback strategy",
        &context
    ).await?;
    ...
    let monitor = tokio::spawn(async move {
        while let Some(event) = event_rx.recv().await {
            match event {
                ...
                ExecutionEvent::StepCompleted { step_index, result, .. } => {
    ...
    for trigger in &triggers {
        ...
        if composite_replanner.should_replan(&replan_plan, trigger).await {
    ...
    for step in &priority_plan.steps {
        ...
        println!("  {} [{}] {}", emoji, format!("{:?}", step.priority).to_uppercase(), step.description);
    }
    println!();
    ...
    plan.metadata.insert("version".to_string(), serde_json::json!("1.0.0"));
    plan.metadata.insert("environment".to_string(), serde_json::json!("production"));
    ...
    println!("Restored plan: {} with {} steps\n", restored.name, restored.steps.len());
