...
use beebotos_agents::{
    Decomposer, DecompositionContext, DecompositionStrategy, ExecutionConfig,
    ExecutionStrategy, Plan, PlanContext, PlanExecutor, PlanStatus, PlanStep,
    PlanStrategy, Planner, PlanningEngine, Priority, StepType,
    ReActPlanner, GoalBasedPlanner, ChainOfThoughtPlanner, HybridPlanner,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plan = engine.quick_plan("Analyze system performance and generate optimization report").await?;
    ...
    // ReAct Strategy
    let react_plan = engine.create_plan(
    ...
    let cot_plan = engine.create_plan(
    ...
    // Goal-Based Strategy
    let goal_plan = engine.create_plan(
    ...
    let step3 = custom_plan.add_step_with_deps(
        PlanStep::new("test", "Test services"),
        vec![step1, step2]
    )?;
    let _ = custom_plan.add_step_with_deps(
        PlanStep::new("deploy", "Deploy to production"),
        vec![step3]
    )?;
