...
use beebotos_message_bus::{Message, MessageBus, DefaultMessageBus, JsonCodec, MemoryTransport};
use serde_json::json;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=3 {
        ...
        match bus.request("rpc/echo", request, Duration::from_secs(5)).await {
            Ok(response) => {
    ...
    for calc in calculations {
        ...
        match bus.request("rpc/calc", request, Duration::from_secs(5)).await {
            Ok(response) => {
    ...
    match bus.request("rpc/nonexistent", slow_request, Duration::from_millis(500)).await {
        ...
        while let Some(request) = stream.recv().await {
            if let Some(reply_to) = request.metadata.headers.get("reply_to") {
                ...
                let req_json: serde_json::Value = 
                    serde_json::from_slice(&request.payload).unwrap_or(json!({"action": "unknown"}));
    ...
    if let Ok(response) = bus.request("api/users", request, Duration::from_secs(5)).await {
        let resp_json: serde_json::Value = serde_json::from_slice(&response.payload)?;
        println!("   [Client] Response: {}", serde_json::to_string_pretty(&resp_json)?);
    }
