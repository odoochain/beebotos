...
pub async fn execute(args: MessageArgs) -> Result<()> {
    ...
    match args.command {
        ...
        MessageCommand::Send { to, channel, message, timeout, reply_to, attachment } => {
            let target = to.or(channel).ok_or_else(|| anyhow::anyhow!("Either --to or --channel must be specified"))?;
            let progress = TaskProgress::new(format!("Sending message to {}", target));
            
            let response = client.send_message_advanced(
                &target, 
                &message, 
                timeout,
                reply_to.as_deref(),
        ...
        MessageCommand::Broadcast { capability, channels, message } => {
            ...
            let recipients = if !channels.is_empty() {
                ...
                client.broadcast_message(capability.as_deref(), &message).await?
            };
            
            progress.finish_success(Some(&format!("{} recipients", recipients.len())));
        }
        
        MessageCommand::Edit { message_id, channel, content } => {
            let progress = TaskProgress::new("Editing message");
            client.edit_message(&message_id, channel.as_deref(), &content).await?;
            progress.finish_success(Some("Message updated"));
        }
        
        MessageCommand::Delete { message_id, channel } => {
            let progress = TaskProgress::new("Deleting message");
            client.delete_message(&message_id, channel.as_deref()).await?;
            progress.finish_success(Some("Message deleted"));
        }
        
        MessageCommand::Read { channel, message_id } => {
            let progress = TaskProgress::new("Marking as read");
            client.mark_as_read(&channel, message_id.as_deref()).await?;
            progress.finish_success(Some(if message_id.is_some() { "Message marked as read" } else { "All messages marked as read" }));
        }
        ...
        MessageCommand::History { agent, channel, limit, since } => {
            let history = if let Some(ch) = channel {
                client.get_channel_message_history(&ch, limit, since.as_deref()).await?
            } else {
        ...
        MessageCommand::React(cmd) => match cmd {
            ReactCommand::Add { message_id, channel, emoji } => {
            ...
            ReactCommand::Remove { message_id, channel, emoji } => {
                let progress = TaskProgress::new("Removing reaction");
                client.remove_reaction(&message_id, &channel, emoji.as_deref()).await?;
                progress.finish_success(Some("Reaction removed"));
            }
            ReactCommand::List { message_id, channel } => {
        ...
        MessageCommand::Poll(cmd) => match cmd {
            PollCommand::Create { channel, question, options, multiple, duration } => {
                let progress = TaskProgress::new("Creating poll");
                let poll_id = client.create_poll(&channel, &question, &options, multiple, duration).await?;
        ...
        MessageCommand::Thread(cmd) => match cmd {
            ThreadCommand::Create { channel, name, parent_message } => {
                let progress = TaskProgress::new("Creating thread");
                let thread_id = client.create_thread(&channel, &name, parent_message.as_deref()).await?;
            ...
            ThreadCommand::List { channel, archived } => {
                ...
                for thread in threads {
                    ...
                    println!("{} {} - {} messages", status, thread.name, thread.message_count);
                }
            }
        ...
        MessageCommand::Role(cmd) => match cmd {
            ...
            RoleCommand::Add { channel, user, role } => {
            ...
            RoleCommand::Remove { channel, user, role } => {
                let progress = TaskProgress::new("Removing role");
                client.remove_role_from_member(&channel, &user, &role).await?;
        ...
        MessageCommand::Member(cmd) => match cmd {
            MemberCommand::List { channel } => {
                ...
                for member in members {
                    println!("  {} (@{}) - {}", member.display_name, member.username, member.role);
                }
            }
            ...
            MemberCommand::Kick { channel, user, reason } => {
                let progress = TaskProgress::new("Kicking member");
                client.kick_member(&channel, &user, reason.as_deref()).await?;
                progress.finish_success(Some(&format!("Kicked {}", user)));
            }
            MemberCommand::Ban { channel, user, reason, delete_days } => {
                let progress = TaskProgress::new("Banning member");
                client.ban_member(&channel, &user, reason.as_deref(), delete_days).await?;
            ...
            MemberCommand::Timeout { channel, user, duration, reason } => {
                let progress = TaskProgress::new("Setting timeout");
                client.timeout_member(&channel, &user, duration, reason.as_deref()).await?;
        ...
        MessageCommand::Media(cmd) => match cmd {
            MediaCommand::Emoji(cmd) => match cmd {
                ...
                EmojiCommand::Upload { channel, name, image } => {
            ...
            MediaCommand::Sticker(cmd) => match cmd {
                ...
                StickerCommand::Upload { channel, name, image, emoji } => {
                    let progress = TaskProgress::new("Uploading sticker");
                    client.upload_sticker(&channel, &name, &image, &emoji).await?;
        ...
        MessageCommand::Event(cmd) => match cmd {
            ...
            EventCommand::Create { channel, name, description, start, end } => {
                let progress = TaskProgress::new("Creating event");
                let event_id = client.create_event(&channel, &name, description.as_deref(), &start, end.as_deref()).await?;
...
trait MessageClient {
    async fn send_message_advanced(&self, target: &str, message: &str, timeout: u64, reply_to: Option<&str>, attachments: &[PathBuf]) -> Result<String>;
    async fn broadcast_to_channels(&self, channels: &[String], message: &str) -> Result<Vec<String>>;
    async fn edit_message(&self, message_id: &str, channel: Option<&str>, content: &str) -> Result<()>;
    ...
    async fn get_channel_message_history(&self, channel: &str, limit: usize, since: Option<&str>) -> Result<Vec<MessageInfo>>;
    
    // Reactions
    async fn add_reaction(&self, message_id: &str, channel: &str, emoji: &str) -> Result<()>;
    async fn remove_reaction(&self, message_id: &str, channel: &str, emoji: Option<&str>) -> Result<()>;
    async fn list_reactions(&self, message_id: &str, channel: &str) -> Result<Vec<(String, u32)>>;
    
    // Polls
    async fn create_poll(&self, channel: &str, question: &str, options: &[String], multiple: bool, duration: Option<u32>) -> Result<String>;
    ...
    // Threads
    async fn create_thread(&self, channel: &str, name: &str, parent_message: Option<&str>) -> Result<String>;
    ...
    async fn ban_member(&self, channel: &str, user: &str, reason: Option<&str>, delete_days: u8) -> Result<()>;
    async fn unban_member(&self, channel: &str, user: &str) -> Result<()>;
    async fn timeout_member(&self, channel: &str, user: &str, duration: u64, reason: Option<&str>) -> Result<()>;
    ...
    async fn upload_sticker(&self, channel: &str, name: &str, image: &PathBuf, emoji: &str) -> Result<()>;
    ...
    async fn create_event(&self, channel: &str, name: &str, description: Option<&str>, start: &str, end: Option<&str>) -> Result<String>;
...
impl MessageClient for crate::client::ApiClient {
    async fn send_message_advanced(&self, _target: &str, _message: &str, _timeout: u64, _reply_to: Option<&str>, _attachments: &[PathBuf]) -> Result<String> {
        // Implementation would send message with attachments
        Ok("msg_123".to_string())
    }
    
    async fn broadcast_to_channels(&self, _channels: &[String], _message: &str) -> Result<Vec<String>> {
        Ok(vec![])
    }
    
    async fn edit_message(&self, _message_id: &str, _channel: Option<&str>, _content: &str) -> Result<()> {
        Ok(())
    }
    ...
    async fn get_channel_message_history(&self, _channel: &str, _limit: usize, _since: Option<&str>) -> Result<Vec<MessageInfo>> {
        Ok(vec![])
    }
    ...
    async fn remove_reaction(&self, _message_id: &str, _channel: &str, _emoji: Option<&str>) -> Result<()> {
        Ok(())
    }
    
    async fn list_reactions(&self, _message_id: &str, _channel: &str) -> Result<Vec<(String, u32)>> {
        Ok(vec![])
    }
    
    // Polls
    async fn create_poll(&self, _channel: &str, _question: &str, _options: &[String], _multiple: bool, _duration: Option<u32>) -> Result<String> {
    ...
    // Threads
    async fn create_thread(&self, _channel: &str, _name: &str, _parent_message: Option<&str>) -> Result<String> {
    ...
    async fn get_role_info(&self, _channel: &str, _role_id: &str) -> Result<RoleInfo> {
        Ok(RoleInfo { name: "Member".to_string(), member_count: 0, permissions: vec![] })
    }
    ...
    async fn remove_role_from_member(&self, _channel: &str, _user: &str, _role: &str) -> Result<()> {
        Ok(())
    }
    ...
    async fn ban_member(&self, _channel: &str, _user: &str, _reason: Option<&str>, _delete_days: u8) -> Result<()> {
        Ok(())
    }
    ...
    async fn timeout_member(&self, _channel: &str, _user: &str, _duration: u64, _reason: Option<&str>) -> Result<()> {
        Ok(())
    }
    ...
    async fn upload_sticker(&self, _channel: &str, _name: &str, _image: &PathBuf, _emoji: &str) -> Result<()> {
        Ok(())
    }
    
    async fn get_voice_status(&self, _channel: &str) -> Result<VoiceStatus> {
        Ok(VoiceStatus { connected: false, participants: 0 })
    }
    ...
    async fn create_event(&self, _channel: &str, _name: &str, _description: Option<&str>, _start: &str, _end: Option<&str>) -> Result<String> {
