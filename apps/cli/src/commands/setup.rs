...
"#);
    ...
    let providers = vec!["OpenAI", "Anthropic", "DeepSeek", "Kimi", "Ollama (Local)", "Skip for now"];
    ...
    println!("Telegram: {}", if telegram_token.is_some() { "Configured" } else { "Not configured" });
    ...
    if api_key.is_some() {
        let env_var = format!("{}_API_KEY", providers[provider_idx].to_uppercase().replace(" ", "_"));
...
async fn perform_environment_checks() -> Result<Vec<EnvironmentCheck>> {
    ...
    checks.push(EnvironmentCheck {
        ...
        passed: home_check,
        message: if home_check { None } else { Some("Cannot access home directory".to_string()) },
    });
    ...
    checks.push(EnvironmentCheck {
        ...
        message: if port_available { None } else { Some("Port 8080 is in use".to_string()) },
    });
