...
pub async fn execute(args: SecurityArgs) -> Result<()> {
    ...
    match args.command {
        SecurityCommand::Status => {
            ...
            println!("{}", "=".repeat(50));
            println!("Overall: {}", if status.secure { "✅ Secure" } else { "⚠️  Issues Found" });
            println!("Audit Log: {}", if status.audit_enabled { "✅ Enabled" } else { "❌ Disabled" });
            println!("Secrets Vault: {}", if status.secrets_encrypted { "🔐 Encrypted" } else { "⚠️  Unencrypted" });
            println!("Last Scan: {}", status.last_scan.as_deref().unwrap_or("Never"));
            println!();
        ...
        SecurityCommand::Scan { scope, format, output } => {
            ...
            PolicyCommand::List => {
                ...
                if policies.is_empty() {
                    ...
                    for policy in policies {
                        ...
                        println!("     Status: {}", if policy.active { "Active" } else { "Inactive" });
                        println!();
                    }
                }
            }
        ...
        SecurityCommand::Audit { command } => match command {
            AuditCommand::List { agent, action, limit } => {
                let logs = client.list_audit_logs(agent.as_deref(), action.as_deref(), limit).await?;
            ...
            AuditCommand::Export { output, format, from, to } => {
                let progress = TaskProgress::new("Exporting audit logs");
                let logs = client.export_audit_logs(from.as_deref(), to.as_deref()).await?;
        ...
        SecurityCommand::Secret { command } => match command {
            ...
            SecretCommand::Set { name, value, namespace, generate, length } => {
                ...
                client.set_secret(&name, &secret_value, namespace.as_deref()).await?;
            ...
            SecretCommand::Delete { name, namespace, force } => {
                if !force {
                    let confirm = dialoguer::Confirm::new()
                        .with_prompt(format!("Delete secret '{}' from namespace '{}'?", name, namespace.as_deref().unwrap_or("default")))
                        .interact()?;
            ...
            SecretCommand::Import { file, namespace } => {
                ...
                let count = client.import_secrets(&secrets, namespace.as_deref()).await?;
                println!("✅ Imported {} secrets", count);
            }
            SecretCommand::Export { file, namespace, passphrase } => {
        ...
        SecurityCommand::Acl { command } => match command {
            AclCommand::List { subject, resource } => {
                let entries = client.list_acl(subject.as_deref(), resource.as_deref()).await?;
                
                println!("ACL Entries:");
                println!("{}", "-".repeat(60));
                for entry in entries {
                    println!("{} -> {} : {}", entry.subject, entry.resource, entry.permission);
                }
            }
            AclCommand::Grant { subject, resource, permission } => {
                client.grant_permission(&subject, &resource, &permission).await?;
                println!("✅ Granted {} permission on {} to {}", permission, resource, subject);
            }
            AclCommand::Revoke { subject, resource, permission } => {
                client.revoke_permission(&subject, &resource, permission.as_deref()).await?;
                if let Some(p) = permission {
                    println!("✅ Revoked {} permission on {} from {}", p, resource, subject);
                } else {
                    println!("✅ Revoked all permissions on {} from {}", resource, subject);
                }
            }
            AclCommand::Check { subject, resource, permission } => {
                let allowed = client.check_permission(&subject, &resource, &permission).await?;
                if allowed {
                    println!("✅ {} has '{}' permission on {}", subject, permission, resource);
                } else {
                    println!("❌ {} does NOT have '{}' permission on {}", subject, permission, resource);
                }
            }
            AclCommand::Roles { command } => match command {
                ...
                Some(RoleCommand::Create { name, permissions }) => {
                    ...
                    println!("✅ Role '{}' created with {} permissions", name, perms.len());
                }
...
trait SecurityClient {
    ...
    async fn list_audit_logs(&self, agent: Option<&str>, action: Option<&str>, limit: usize) -> Result<Vec<AuditLog>>;
    async fn export_audit_logs(&self, from: Option<&str>, to: Option<&str>) -> Result<Vec<AuditLog>>;
    ...
    async fn import_secrets(&self, secrets: &serde_json::Value, namespace: Option<&str>) -> Result<usize>;
    async fn export_secrets(&self, namespace: Option<&str>) -> Result<String>;
    async fn list_acl(&self, subject: Option<&str>, resource: Option<&str>) -> Result<Vec<AclEntry>>;
    async fn grant_permission(&self, subject: &str, resource: &str, permission: &str) -> Result<()>;
    async fn revoke_permission(&self, subject: &str, resource: &str, permission: Option<&str>) -> Result<()>;
    async fn check_permission(&self, subject: &str, resource: &str, permission: &str) -> Result<bool>;
...
impl SecurityClient for crate::client::ApiClient {
    ...
    async fn export_audit_logs(&self, _from: Option<&str>, _to: Option<&str>) -> Result<Vec<AuditLog>> {
    ...
    async fn delete_secret(&self, name: &str, namespace: Option<&str>) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Delete secret failed ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn import_secrets(&self, _secrets: &serde_json::Value, _namespace: Option<&str>) -> Result<usize> {
