...
pub async fn execute(args: BrowserArgs) -> Result<()> {
    ...
    match args.command {
        ...
        BrowserCommand::Start { headless, user_data_dir, proxy } => {
            let progress = TaskProgress::new("Starting browser");
            client.start_browser(headless, user_data_dir.as_ref(), proxy.as_deref()).await?;
        ...
        BrowserCommand::Navigate { url, wait, wait_for } => {
            let progress = TaskProgress::new(format!("Navigating to {}", url));
            client.browser_navigate(&url, wait, wait_for.as_deref()).await?;
            progress.finish_success(None);
            println!("✅ Navigated to {}", url);
        }
        
        BrowserCommand::Screenshot { output, full_page, selector } => {
            let progress = TaskProgress::new("Taking screenshot");
            client.browser_screenshot(&output, full_page, selector.as_deref()).await?;
            progress.finish_success(None);
        ...
        BrowserCommand::Click { selector, timeout } => {
            ...
            client.browser_click(&selector, Duration::from_secs(timeout)).await?;
            progress.finish_success(None);
            println!("✅ Clicked {}", selector);
        }
        
        BrowserCommand::Type { selector, text, clear } => {
        ...
        BrowserCommand::Wait { selector, timeout } => {
            ...
            client.browser_wait_for(&selector, Duration::from_secs(timeout)).await?;
            progress.finish_success(None);
        ...
        BrowserCommand::Tab(cmd) => match cmd {
            TabCommand::List => {
                ...
                for (i, tab) in tabs.iter().enumerate() {
                    ...
                    println!("{} [{}] {} - {}", active, i, &tab.title[..tab.title.len().min(40)], &tab.url[..tab.url.len().min(40)]);
                }
            }
            TabCommand::Open { url } => {
...
trait BrowserClient {
    ...
    async fn start_browser(&self, headless: bool, user_data_dir: Option<&PathBuf>, proxy: Option<&str>) -> Result<()>;
    ...
    async fn browser_screenshot(&self, output: &PathBuf, full_page: bool, selector: Option<&str>) -> Result<()>;
...
impl BrowserClient for crate::client::ApiClient {
    ...
    async fn start_browser(&self, headless: bool, _user_data_dir: Option<&PathBuf>, _proxy: Option<&str>) -> Result<()> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(anyhow::anyhow!("Failed to start browser ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn stop_browser(&self) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to stop browser ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn reset_browser_profile(&self, name: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to reset profile ({}): {}", status, text));
        }
        Ok(())
    }
    
    async fn browser_navigate(&self, url: &str, _wait: bool, _wait_for: Option<&str>) -> Result<()> {
    ...
    async fn browser_screenshot(&self, _output: &PathBuf, _full_page: bool, _selector: Option<&str>) -> Result<()> {
    ...
    async fn browser_open_tab(&self, url: Option<&str>) -> Result<TabInfo> {
        ...
        if !resp.status().is_success() {
            return Ok(TabInfo { id: "0".to_string(), title: "New Tab".to_string(), url: "about:blank".to_string(), active: false });
        }
    ...
    async fn browser_create_profile(&self, name: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Create profile failed ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn browser_delete_profile(&self, name: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Delete profile failed ({}): {}", status, text));
        }
        Ok(())
    }
