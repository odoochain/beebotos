...
pub async fn execute(args: AgentArgs) -> Result<()> {
    ...
    match args.command {
        ...
        AgentCommand::List { status, all, capability } => {
        ...
        AgentCommand::Stop { id, force, graceful_timeout } => {
            let progress = TaskProgress::new(format!("Stopping agent {}", id));
            client.stop_agent_graceful(&id, force, graceful_timeout).await?;
            progress.finish_success(None);
        }
        ...
        AgentCommand::Pause { id } => {
            ...
            println!("⏸️  Agent paused. Use `beebot agent resume {}` to resume.", id);
        }
        ...
        AgentCommand::Delete { id, force, cleanup_chain } => {
            if !force {
        ...
        AgentCommand::Clone { source, name, with_memory } => {
            let progress = TaskProgress::new(format!("Cloning agent {}", source));
            let new_agent = client.clone_agent(&source, name.as_deref(), with_memory).await?;
        ...
        AgentCommand::Export { id, output, include_memory, include_keys } => {
            let progress = TaskProgress::new(format!("Exporting agent {}", id));
            let export_data = client.export_agent(&id, include_memory, include_keys).await?;
            progress.finish_success(None);
        ...
        AgentCommand::Channel(cmd) => match cmd {
            ...
            AgentChannelCommand::Bind { id, channel, config } => {
                let progress = TaskProgress::new(format!("Binding to {}", channel));
                client.bind_agent_to_channel(&id, &channel, config.as_deref()).await?;
                progress.finish_success(None);
        ...
        AgentCommand::SetIdentity { id, did, personality } => {
            ...
            if let Some(personality_path) = personality {
                ...
                client.set_agent_personality(&id, &personality_config).await?;
        ...
        AgentCommand::Exec { id, input, timeout, sandbox } => {
            let progress = TaskProgress::new(format!("Executing task on agent {}", id));
            let result = client.exec_task_with_sandbox(&id, &input, timeout, &sandbox).await?;
            progress.finish_success(None);
            println!("Result: {}", result.output);
        }
        
        AgentCommand::Run { agent, message, deliver, channel, timeout, sandbox } => {
            ...
            let agent_id = match agent {
                ...
                None => {
                    ...
                    agents.first().map(|a| a.id.clone()).ok_or_else(||
                        anyhow::anyhow!("No running agent found. Specify --agent or start an agent.")
                    )?
                }
            };
            
            let result = client.exec_task_with_sandbox(&agent_id, &message, timeout, &sandbox).await?;
            progress.finish_success(None);
            ...
            if deliver {
                if let Some(ch) = channel {
                    client.deliver_to_channel(&agent_id, &ch, &result.output).await?;
...
trait AgentClient {
    ...
    async fn list_agents_by_capability(&self, capabilities: &[String]) -> Result<Vec<crate::client::AgentInfo>>;
    ...
    async fn clone_agent(&self, source: &str, name: Option<&str>, with_memory: bool) -> Result<crate::client::AgentInfo>;
    async fn export_agent(&self, agent_id: &str, include_memory: bool, include_keys: bool) -> Result<serde_json::Value>;
    async fn import_agent(&self, config: &str, name: Option<&str>) -> Result<crate::client::AgentInfo>;
    ...
    async fn bind_agent_to_channel(&self, agent_id: &str, channel: &str, config: Option<&str>) -> Result<()>;
    ...
    async fn exec_task_with_sandbox(&self, agent_id: &str, input: &str, timeout: u64, sandbox: &str) -> Result<crate::client::TaskResult>;
...
impl AgentClient for crate::client::ApiClient {
    ...
    async fn list_agents_by_capability(&self, _capabilities: &[String]) -> Result<Vec<crate::client::AgentInfo>> {
    ...
    async fn pause_agent(&self, agent_id: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(anyhow::anyhow!("Failed to pause agent ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn resume_agent(&self, agent_id: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to resume agent ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn clone_agent(&self, source: &str, name: Option<&str>, with_memory: bool) -> Result<crate::client::AgentInfo> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to clone agent ({}): {}", status, text));
        }
    ...
    async fn export_agent(&self, agent_id: &str, include_memory: bool, include_keys: bool) -> Result<serde_json::Value> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to export agent ({}): {}", status, text));
        }
    ...
    async fn import_agent(&self, config: &str, name: Option<&str>) -> Result<crate::client::AgentInfo> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to import agent ({}): {}", status, text));
        }
    ...
    async fn bind_agent_to_channel(&self, agent_id: &str, channel: &str, config: Option<&str>) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to bind channel ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn unbind_agent_from_channel(&self, agent_id: &str, channel: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to unbind channel ({}): {}", status, text));
        }
        Ok(())
    }
    ...
    async fn set_agent_personality(&self, agent_id: &str, config: &str) -> Result<()> {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("Failed to set personality ({}): {}", status, text));
        }
        Ok(())
    }
    
    async fn exec_task_with_sandbox(&self, agent_id: &str, input: &str, timeout: u64, _sandbox: &str) -> Result<crate::client::TaskResult> {
        // Would apply sandbox level
        self.exec_task(agent_id, input, timeout).await
    }
    
    async fn deliver_to_channel(&self, _agent_id: &str, _channel: &str, _content: &str) -> Result<()> {
