...
pub async fn execute(args: GatewayArgs) -> Result<()> {
    ...
    match args.command {
        GatewayCommand::Run { bind, kernel_mode, debug, config } => {
            ...
            match client.run_gateway(&bind, kernel_mode, debug, config.as_ref()).await {
                Ok(_) => {
        ...
        GatewayCommand::Install { service_type, auto_start } => {
        ...
        GatewayCommand::Start { wait, timeout } => {
            ...
            if wait {
                println!("⏳ Waiting for gateway to be ready (timeout: {}s)...", timeout);
                    ...
                    match client.probe_gateway("http://localhost:8080", 2).await {
                        ...
                        Ok(false) => {
                            if start.elapsed() > timeout_duration {
                                ...
                                return Err(anyhow!("Gateway failed to start within {} seconds", timeout));
                            }
        ...
        GatewayCommand::Health { verbose, check_chain } => {
            ...
            if verbose {
                ...
                if check_chain {
                    println!("\n⛓️  Blockchain:");
                    println!("  Connected: {}", if health.chain.connected { "✅" } else { "❌" });
        ...
        GatewayCommand::Call { method, params, gateway } => {
            let endpoint = gateway.as_deref().unwrap_or("http://localhost:8080");
            
            println!("📞 Calling {} on {}...", method, endpoint);
            
            let result = client.call_gateway_rpc(endpoint, &method, params.as_deref()).await?;
        ...
        GatewayCommand::Logs { follow, lines, level } => {
            if follow {
    ...
    match format {
        OutputFormat::Table => {
            ...
            if status.running {
                ...
                println!("\n💻 System Resources:");
                println!("  Memory: {} MB / {} MB", status.resources.memory_used_mb, status.resources.memory_total_mb);
...
trait GatewayClient {
    async fn run_gateway(&self, bind: &str, kernel_mode: bool, debug: bool, config: Option<&PathBuf>) -> Result<()>;
    ...
    async fn discover_gateways(&self, method: &str, timeout: u64) -> Result<Vec<DiscoveredGateway>>;
    async fn call_gateway_rpc(&self, endpoint: &str, method: &str, params: Option<&str>) -> Result<serde_json::Value>;
...
impl GatewayClient for crate::client::ApiClient {
    async fn run_gateway(&self, _bind: &str, _kernel_mode: bool, _debug: bool, _config: Option<&PathBuf>) -> Result<()> {
        // This would actually spawn the gateway process
        // For now, return not implemented
        anyhow::bail!("Running gateway in foreground is not yet implemented. Use 'gateway start' instead.")
    }

    async fn install_gateway(&self, service_type: &str, auto_start: bool) -> Result<()> {
        let url = format!("{}/gateway/install", self.build_url("").trim_end_matches('/'));
    ...
    async fn uninstall_gateway(&self, keep_data: bool) -> Result<()> {
        let url = format!("{}/gateway/uninstall?keep_data={}", self.build_url("").trim_end_matches('/'), keep_data);
        let resp = self.http().post(&url).headers(self.headers()?).send().await?;
        
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow!("Failed to uninstall gateway ({}): {}", status, text));
        }
    ...
    async fn stop_gateway_service(&self, force: bool) -> Result<()> {
        let url = format!("{}/gateway/stop?force={}", self.build_url("").trim_end_matches('/'), force);
        let resp = self.http().post(&url).headers(self.headers()?).send().await?;
    ...
    async fn restart_gateway_service(&self, graceful: bool) -> Result<()> {
        let url = format!("{}/gateway/restart?graceful={}", self.build_url("").trim_end_matches('/'), graceful);
        let resp = self.http().post(&url).headers(self.headers()?).send().await?;
    ...
    async fn get_gateway_status(&self) -> Result<GatewayStatus> {
        let url = format!("{}/gateway/status", self.build_url("").trim_end_matches('/'));
        let resp = self.http().get(&url).headers(self.headers()?).send().await?;
        
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow!("Failed to get gateway status ({}): {}", status, text));
        }
    ...
    async fn check_gateway_health(&self, check_chain: bool) -> Result<GatewayHealth> {
        let url = format!("{}/gateway/health?check_chain={}", self.build_url("").trim_end_matches('/'), check_chain);
        let resp = self.http().get(&url).headers(self.headers()?).send().await?;
    ...
    async fn discover_gateways(&self, method: &str, timeout: u64) -> Result<Vec<DiscoveredGateway>> {
        let url = format!("{}/gateway/discover?method={}&timeout={}", self.build_url("").trim_end_matches('/'), method, timeout);
        let resp = self.http().get(&url).headers(self.headers()?).send().await?;
    ...
    async fn get_gateway_usage(&self, range: &str, by_agent: bool) -> Result<UsageStats> {
        let url = format!("{}/gateway/usage?range={}&by_agent={}", self.build_url("").trim_end_matches('/'), range, by_agent);
        let resp = self.http().get(&url).headers(self.headers()?).send().await?;
    ...
    async fn get_gateway_logs(&self, lines: usize, level: Option<&str>) -> Result<Vec<LogEntry>> {
        let mut url = format!("{}/gateway/logs?lines={}", self.build_url("").trim_end_matches('/'), lines);
        if let Some(l) = level {
    ...
    async fn follow_gateway_logs(&self, level: Option<&str>) -> Result<()> {
        let mut url = format!("{}/gateway/logs/stream", self.build_url("").trim_end_matches('/'));
        if let Some(l) = level {
            url.push_str(&format!("?level={}", l));
        }
    ...
    async fn upgrade_gateway(&self, version: Option<&str>, hot: bool) -> Result<UpgradeResult> {
        let url = format!("{}/gateway/upgrade", self.build_url("").trim_end_matches('/'));
        let body = serde_json::json!({
            "version": version,
            "hot": hot,
        });
