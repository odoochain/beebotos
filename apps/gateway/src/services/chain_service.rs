...
use beebotos_chain::{
    compat::{
        Address, ChainClientTrait, AgentIdentityInfo, Bytes, U256, TxHash
    },
...
use super::chain_events::{ChainEventManager, ChainEvent};
        ...
        let parse_address = |addr: &Option<String>| -> Option<Address> {
            addr.as_ref().and_then(|a| {
                ...
                if a.len() >= 42 && a.starts_with("0x") {
                    hex::decode(&a[2..]).ok().filter(|b| b.len() == 20).map(|b| Address::from_slice(&b))
                } else {
                    None
                }
            })
...
impl ChainService {
    ...
    ///
    /// # Arguments
    /// * `agent_id` - Unique agent identifier (string, will be hashed to bytes32)
    ...
    pub async fn register_agent_identity(
        ...
        let identity_contract = self.config.identity_contract.as_ref()
            .ok_or_else(|| AppError::Configuration("Identity contract address not configured".into()))?;
    ...
    pub async fn get_agent_identity(&self, agent_id: &str) -> Result<AgentInfo, AppError> {
        ...
        let identity_info = client.get_agent_identity(
            identity_contract.clone(),
            agent_id_bytes,
        ).await.map_err(|e| AppError::Chain(format!("Failed to get agent identity: {}", e)))?;

        match identity_info {
            Some(info) => {
                // Cache the result
                self.identity_cache.put_identity(agent_id, info.clone()).await;
                
                // Also cache the DID mapping
                self.identity_cache.put_agent_id_by_did(&info.did, agent_id).await;
    ...
    pub async fn has_agent_identity(&self, agent_id: &str) -> bool {
        ...
        let is_registered = match client.is_agent_registered(identity_contract.clone(), agent_id_bytes).await {
            Ok(registered) => {
                // Cache the result
                self.identity_cache.put_registration_status(agent_id, registered).await;
                registered
            }
            Err(e) => {
    ...
    pub async fn get_agent_by_did(&self, did: &str) -> Result<Option<String>, AppError> {
        ...
        let agent_id = client.get_agent_id_by_did(
            identity_contract.clone(),
            did,
        ).await.map_err(|e| AppError::Chain(format!("Failed to get agent by DID: {}", e)))?;

        // Cache the result
        if let Some(ref id) = agent_id {
            let agent_id_hex = format!("0x{}", hex::encode(id));
            self.identity_cache.put_agent_id_by_did(did, &agent_id_hex).await;
        }
    ...
    pub async fn create_dao_proposal(
        ...
        let dao_contract = self.config.dao_contract.as_ref()
            .ok_or_else(|| AppError::Configuration("DAO contract address not configured".into()))?;
    ...
    pub async fn get_proposal(&self, proposal_id: ProposalId) -> Result<ProposalInfo, AppError> {
        ...
        let proposal = client.get_proposal(dao_contract.clone(), proposal_id)
            .await
    ...
    pub async fn list_proposals(
        ...
        let proposals = client.list_proposals(dao_contract.clone(), start_id, limit)
            .await
    ...
    pub async fn get_voting_power(&self, address: Address) -> Result<u64, AppError> {
        ...
        let voting_power = client.get_voting_power(dao_contract.clone(), address)
            .await
    ...
    pub async fn execute_proposal(&self, proposal_id: ProposalId) -> Result<TxHash, AppError> {
        ...
        Err(AppError::NotImplemented("Proposal execution not yet implemented".into()))
    }
    ...
    pub async fn purchase_skill_nft(&self, _skill_id: &str) -> Result<TxHash, AppError> {
        ...
        Err(AppError::NotImplemented("SkillNFT purchase coming soon".into()))
    }
    ...
    pub async fn get_skill_nft(&self, _token_id: u64) -> Result<SkillNFTInfo, AppError> {
        ...
        Err(AppError::NotImplemented("SkillNFT query coming soon".into()))
    }
...
/// 
/// Encodes the registerAgent function call using the AgentIdentity contract ABI.
...
async fn estimate_gas_for_call(
    ...
    let call = beebotos_chain::compat::ContractCall::new(to, Bytes::from(data))
        .with_from(from);
...
async fn initialize_wallet(
    ...
    let account = hd_wallet.derive_account(0, Some("Default Account".to_string()))
        .map_err(|e| beebotos_chain::ChainError::Wallet(format!("Failed to derive account: {}", e)))?;
...
mod tests {
    ...
    fn test_chain_service_config_from_blockchain_config() {
        ...
        assert_eq!(service_config.wallet_mnemonic, Some("test mnemonic".to_string()));
    }
}
