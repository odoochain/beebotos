...
impl BeeHubClient {
    ...
    pub fn new() -> Result<Self, HubError> {
        let base_url = std::env::var("BEEHUB_URL")
    ...
    pub async fn get_skill(&self, id: &str) -> Result<SkillMetadata, HubError> {
        ...
        match resp.status() {
            ...
            reqwest::StatusCode::NOT_FOUND => {
                Err(HubError::NotFound(id.to_string()))
            }
            status => {
    ...
    pub async fn publish_skill(
        ...
        let req = self.build_request(reqwest::Method::POST, "/publish")
            .multipart(
        ...
        if !resp.status().is_success() {
            return Err(HubError::InvalidResponse(
                format!("Publish failed: {}", resp.status())
            ));
        }
